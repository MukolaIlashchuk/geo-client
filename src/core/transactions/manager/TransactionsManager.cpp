    ...
    BaseUserCommand::Shared command)
{
    ...
    } else if (command->identifier() == CreditUsageMultyCommand::identifier()) {
        launchCoordinatorPaymentMultyTransaction(
            dynamic_pointer_cast<CreditUsageMultyCommand>(
                command));
    ...
    SetOutgoingTrustLineCommand::Shared command)
{
    try {
        ...
        auto transaction = make_shared<OpenTrustLineTransaction>(
            mNodeUUID,
            selectEthereumAddress(command->equivalent()),
            mBitcoinPublicKey,
            command,
            mEquivalentsSubsystemsRouter->trustLinesManager(command->equivalent()),
            mStorageHandler,
    ...
    EthereumAuditCommand::Shared command)
{
    try {
        auto transaction = make_shared<EthereumAuditTransaction>(
            ...
            command,
            mEquivalentsSubsystemsRouter->trustLinesManager(command->equivalent()),
            mEquivalentsSubsystemsRouter->ethereumSiganturesManager(command->equivalent()),
            mLog);
        subscribeForProcessingConfirmationMessage(
            transaction->processConfirmationMessageSignal);
        prepareAndSchedule(
    ...
    SetIncomingTrustLineMessage::Shared message)
{
    try {
        auto transaction = make_shared<SetIncomingTrustLineTransaction>(
            mNodeUUID,
            selectEthereumAddress(message->equivalent()),
            message,
...
void TransactionsManager::launchCoordinatorPaymentCrossTransaction(
...
void TransactionsManager::launchCoordinatorPayment2CrossTransaction(
        const NodeUUID &contractorUUID,
        const SerializedEquivalent equivalent,
        const NodeUUID &nextContractorUUID,
        const SerializedEquivalent nextEquivalent,
        const TrustLineAmount &amount)
{
    try {
        auto transaction = make_shared<CoordinatorPaymentTransaction>(
...
void TransactionsManager::launchCoordinatorPaymentMultyTransaction(
        CreditUsageMultyCommand::Shared command)
{
    try {
        auto transaction = make_shared<CoordinatorPaymentTransaction>(
                mNodeUUID,
                command,
                mEquivalentsSubsystemsRouter->trustLinesManager(command->equivalent1()),
                mStorageHandler,
                mEquivalentsSubsystemsRouter->topologyCacheManager(command->equivalent1()),
                mEquivalentsSubsystemsRouter->maxFlowCacheManager(command->equivalent1()),
                mResourcesManager,
                mEquivalentsSubsystemsRouter->pathsManager(command->equivalent1()),
                mKeysStore,
                mPaymentExchangeHandler,
                mLog,
                mSubsystemsController,
        ...
        error() << "There are no subsystems for CoordinatorPaymentTransaction "
                "with equivalent " << command->equivalent1() << " Details are: " << e.what();
        prepareAndSchedule(
                make_shared<NoEquivalentTransaction>(
                        mNodeUUID,
                        command,
...
{
    try {
        TrustLinesManager* crossTrustLinesManager;
        if (message->intermediateEquivalent() != 0) {
            crossTrustLinesManager = mEquivalentsSubsystemsRouter->trustLinesManager(message->intermediateEquivalent());
        } else {
            crossTrustLinesManager = nullptr;
        }
        auto transaction = make_shared<ReceiverPaymentTransaction>(
            mNodeUUID,
            message,
            mEquivalentsSubsystemsRouter->trustLinesManager(message->equivalent()),
            crossTrustLinesManager,
            mStorageHandler,
            mEquivalentsSubsystemsRouter->topologyCacheManager(message->equivalent()),
            mEquivalentsSubsystemsRouter->maxFlowCacheManager(message->equivalent()),
            mKeysStore,
        ...
        subscribeForBuildCyclesThreeNodesTransaction(
            transaction->mBuildCycleThreeNodesSignal);
        subscribeForCrossChainPayment(
            transaction->mCrossChainSignal);
        subscribeForCross2ChainPayment(
            transaction->mCross2ChainSignal);
...
void TransactionsManager::subscribeForCrossChainPayment(
        BasePaymentTransaction::CrossChainSignal &signal)
{
    signal.connect(
            boost::bind(
                    &TransactionsManager::launchCoordinatorPaymentCrossTransaction,
                    this,
                    _1,
                    _2,
                    _3));
...
void TransactionsManager::subscribeForCross2ChainPayment(
        BasePaymentTransaction::Cross2ChainSignal &signal)
{
    signal.connect(
            boost::bind(
                    &TransactionsManager::launchCoordinatorPayment2CrossTransaction,
                    this,
                    _1,
                    _2,
                    _3,
...
string TransactionsManager::selectEthereumAddress(
    SerializedEquivalent equivalent)
{
    if (equivalent == 1) {
        return mEthereumAddress;
    } else if (equivalent == 2) {
        return mErc20Address;
    } else {
        warning() << "selectEthereumAddress: invalid token id " << equivalent;
        return "";
    }
}
