...
TransactionResult::SharedConst OpenTrustLineTransaction::runInitializationStage()
{
    ...
    if (mCommand->isStateChannel() and mEthereumAddress.empty() and mBitcoinPublicKey.empty()) {
...
TransactionResult::SharedConst OpenTrustLineTransaction::runResponseProcessingStage()
{
    ...
    if (mCommand->isStateChannel() and message->ethereumAddress().empty() and message->bitcoinPublicKey().empty()) {
        warning() << "Contractor don't send stete channel data";
        processConfirmationMessage(message);
        return resultDone();
    }
    ...
    if (mCommand->isStateChannel()) {
        if (mEquivalent == 1 or mEquivalent == 2) {
            ...
            pair<eth::ChannelInfo::Shared, bool> channelInfo;
            if (mEquivalent == 1) {
                info() << "get channel info for eth";
                channelInfo = eth::api::channelInfo(channel);
            } else if (mEquivalent == 2) {
                info() << "get channel info for stable coin";
                channelInfo = eth::api::channelInfoErc20(channel);
            } else {
                warning() << "invalid token id " << mEquivalent;
                return resultDone();
            }

//        auto channelInfo = make_pair(
//                make_shared<eth::ChannelInfo>(),
//                true);
//        channelInfo.first->state = 1;
//        channelInfo.first->aliceBalance = mAmount;
            ...
            info() << "state " << channelInfo.first->state;
            info() << "Alice balance " << channelInfo.first->aliceBalance;
            info() << "Bob balance " << channelInfo.first->bobBalance;
        ...
        if (mEquivalent == 3) {
            info() << "contractor bitcoin public key: " << message->bitcoinPublicKey();
            info() << "transactionID: " << message->bitcoinTransactonID();

            mTrustLines->setContractorBitcoinPublicKey(
                ioTransaction,
                mContractorUUID,
                message->bitcoinPublicKey());

            mBitcoinTransactionID = message->bitcoinTransactonID();
            ...
            if (channelInfo.first == "0") {
                info() << "Channel opened but not confirmed";
                mStep = ChannelOpeningPending;
                return resultAwakeAfterMilliseconds(5000);
            }
