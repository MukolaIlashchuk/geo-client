...
AcceptTrustLineTransaction::AcceptTrustLineTransaction(
    ...
    mContractorBitcoinPublcKey(message->bitcoinPublicKey())
{
...
TransactionResult::SharedConst AcceptTrustLineTransaction::runInitializationStage()
{
    ...
    if (!mContractorEthereumAddress.empty() and mEthereumAddress.empty() and mContractorBitcoinPublcKey.empty()) {
    ...
    try {
        ...
        if (!mContractorEthereumAddress.empty()) {
            ...
            pair<string, string> transactionIdAndChannelId;
            if (mEquivalent == 1) {
                info() << "create channel for eth";
                transactionIdAndChannelId = eth::api::createChannel(
                    mContractorEthereumAddress,
                    mAmount);
            } else if (mEquivalent == 2){
                info() << "create channel for stable coin";
                auto amountForApprove = mAmount + mAmount + mAmount;
                auto transactionId = eth::api::tokenApproveErc20(amountForApprove);
                if (!transactionId.second) {
                    warning() << "Can't approve token";
                    return resultDone();
                }

                auto transactionState = eth::api::transactionStatusErc20(transactionId.first);
                while (transactionState.first != "CONFIRMED") {
                    info() << "Transaction state: " << transactionState.first;
                    sleep(5);
                    info() << "get transaction state for approve stable coin";
                    transactionState = eth::api::transactionStatusErc20(transactionId.first);
                }
                info() << "Chanel approved";

                transactionIdAndChannelId = eth::api::createChannelErc20(
                     mContractorEthereumAddress,
                     mAmount);
            } else {
                warning() << "invald token id";
                return resultDone();
            }

//            auto transactionIdAndChannelId = make_pair("", "0xe639afd190e59b93fae71bc4e797839b4f5636fc111222333444555666778899");

            mEthereumChannelId = transactionIdAndChannelId.second;
            info() << "Created channel id " << mEthereumChannelId;
        ...
        if (!mContractorBitcoinPublcKey.empty()) {
            info() << "contractor bitcoin Public Key: " << mContractorBitcoinPublcKey;
            mTrustLines->setContractorBitcoinPublicKey(
                    ioTransaction,
                    mContractorUUID,
                    mContractorBitcoinPublcKey);
            info() << "create channel for btc";
            auto transactionIdAndChannelId = eth::api::createChannelBitcoin(
                    mContractorBitcoinPublcKey,
                    mAmount);

            if (transactionIdAndChannelId.second == "") {
                warning() << "Can't get transactonID: " << transactionIdAndChannelId.first;
                return resultDone();
            }
            mBitcoinTransactionID = transactionIdAndChannelId.first;
            info() << "Transaction ID: " << mBitcoinTransactionID;
            info() << "Channel balance: " << transactionIdAndChannelId.second;
        }
    ...
    if (!mContractorEthereumAddress.empty()) {
        sendMessageWithCaching<TrustLineConfirmationMessage>(
            ...
            "",
            "");
    } else if (!mBitcoinPublicKey.empty()) {
        sendMessageWithCaching<TrustLineConfirmationMessage>(
                mContractorUUID,
                Message::TrustLines_SetIncomingInitial,
                mEquivalent,
