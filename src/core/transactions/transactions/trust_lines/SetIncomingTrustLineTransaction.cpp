...
TransactionResult::SharedConst SetIncomingTrustLineTransaction::runInitializationStage()
{
    ...
    try {
        ...
        if (mTrustLines->isStateChannel(mContractorUUID)) {
            ...
            if (mEquivalent == 1) {
                info() << "accept channel info for eth";
                auto transactionIdAndChannelId = eth::api::acceptChannel(
                    mTrustLines->contractorEthereumAddress(mContractorUUID),
                    mAmount);
            } else if (mEquivalent == 2) {
                info() << "accept channel info for stable coin";
                auto transactionIdAndChannelId = eth::api::acceptChannelErc20(
                        mTrustLines->contractorEthereumAddress(mContractorUUID),
                        mAmount);
            } else {
