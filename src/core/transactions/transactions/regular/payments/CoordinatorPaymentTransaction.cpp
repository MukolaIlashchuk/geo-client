...
CoordinatorPaymentTransaction::CoordinatorPaymentTransaction(
        const NodeUUID &kCurrentNodeUUID,
        const CreditUsageMultyCommand::Shared kCommand,
        TrustLinesManager *trustLines,
        StorageHandler *storageHandler,
        TopologyCacheManager *topologyCacheManager,
        MaxFlowCacheManager *maxFlowCacheManager,
        ...
        BasePaymentTransaction(
                ...
                log,
                subsystemsController),
        mCommand(make_shared<CreditUsageCommand>(
                kCommand->UUID(),
                kCommand->equivalent1(),
                kCommand->contractorUUID(),
                kCommand->amount())),
        mMultyCommand(kCommand),
        mResourcesManager(resourcesManager),
        mPathsManager(pathsManager),
        mReservationsStage(0),
        mDirectPathIsAlreadyProcessed(false),
        ...
        mIntermediateNodeUUID(mMultyCommand->intermediateUUID()),
        mIntermediateEquivalent(mMultyCommand->equivalent2()),
        mNextIntermediateNodeUUID(mMultyCommand->nextIntermediateUUID()),
        mNextIntermediateEquivalent(mMultyCommand->equivalent3())
{
    mStep = Stages::Coordinator_Initialization;
}
...
noexcept :
        ...
        mPreviousInaccessibleNodesCount(0),
        mPreviousRejectedTrustLinesCount(0),
        mIntermediateNodeUUID(nextContractorUUID),
        mIntermediateEquivalent(nextEquivalent),
        mNextIntermediateNodeUUID(NodeUUID::empty()),
...
CoordinatorPaymentTransaction::CoordinatorPaymentTransaction(
        ...
        mPreviousInaccessibleNodesCount(0),
        mPreviousRejectedTrustLinesCount(0),
        mIntermediateNodeUUID(NodeUUID::empty()),
...
TransactionResult::SharedConst CoordinatorPaymentTransaction::runPathsResourceProcessingStage()
{
    ...
    if (mNextIntermediateEquivalent != 0) {
        sendMessage<ReceiverInitPaymentRequestMessage>(
                ...
                mCommand->amount(),
                mIntermediateNodeUUID,
                mIntermediateEquivalent,
                mNextIntermediateNodeUUID,
                mNextIntermediateEquivalent);
        info() << "Next next " << mIntermediateNodeUUID << " " << mIntermediateEquivalent
               << " " << mNextIntermediateNodeUUID << " " << mNextIntermediateEquivalent;
        ...
        sendMessage<ReceiverInitPaymentRequestMessage>(
                ...
                mIntermediateNodeUUID,
                mIntermediateEquivalent);
        info() << "Next " << mIntermediateNodeUUID << " " << mIntermediateEquivalent;
    } else {
        sendMessage<ReceiverInitPaymentRequestMessage>(
                mCommand->contractorUUID(),
                mEquivalent,
