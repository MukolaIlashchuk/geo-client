...
ReceiverPaymentTransaction::ReceiverPaymentTransaction(
    ...
    TrustLinesManager *trustLinesCrossChain,
    ...
    mMessage(message),
    mTrustLinesCrossChain(trustLinesCrossChain),
    mTransactionShouldBeRejected(false),
    mVisualInterface(visualInterface),
    mPaymentExchangeHandler(paymentExchangeHandler)
{
...
TransactionResult::SharedConst ReceiverPaymentTransaction::runInitializationStage()
{
    ...
    if (mMessage->intermediateUUID().stringUUID() != NodeUUID::empty().stringUUID()) {
        info() << "Cross chain next node: " << mMessage->intermediateUUID() << " equivalent " << mMessage->intermediateEquivalent();
        if (!mPaymentExchangeHandler->isRatePresent(mEquivalent, mMessage->intermediateEquivalent())) {
            sendMessage<ReceiverInitPaymentResponseMessage>(
                    kCoordinator,
                    mEquivalent,
                    currentNodeUUID(),
            ...
            info() << "Operation rejected due impossible cross chain: no rate.";
            return resultDone();
        }

        if (!mTrustLinesCrossChain->trustLineIsPresent(mMessage->intermediateUUID())) {
            sendMessage<ReceiverInitPaymentResponseMessage>(
                    kCoordinator,
                    mEquivalent,
                    currentNodeUUID(),
            ...
            info() << "Operation rejected due impossible cross chain: no TL on next equivalent.";
            return resultDone();
        }

        stringstream s;
        s << mMessage->amount();
        auto amountInt = atoi(s.str().c_str());
        auto nextAmountInt = (int)(amountInt * mPaymentExchangeHandler->getRate(mEquivalent, mMessage->intermediateEquivalent()));
        auto nextCrossChainAmount = TrustLineAmount(nextAmountInt);
        if (mTrustLinesCrossChain->incomingTrustAmount(mMessage->intermediateUUID()) < nextCrossChainAmount) {
            sendMessage<ReceiverInitPaymentResponseMessage>(
                    kCoordinator,
                    mEquivalent,
                    currentNodeUUID(),
            ...
            info() << "Operation rejected due impossible cross chain: insufficient funds on next equivalent.";
            info() << "Incoming amount on next equivalent " << mTrustLinesCrossChain->incomingTrustAmount(mMessage->intermediateUUID());
            info() << "Required amount on next equivalent " << nextCrossChainAmount;
            return resultDone();
        }

        mIntermediateUUID = mMessage->intermediateUUID();
        mIntermediateEquivalent = mMessage->intermediateEquivalent();

        if (mMessage->nextIntermediateEquivalent() != 0) {
            mNextIntermediateUUID = mMessage->nextIntermediateUUID();
            mNextIntermediateEquivalent = mMessage->nextIntermediateEquivalent();
            info() << "Cross 2 chain next node: " << mNextIntermediateUUID << " equivalent " << mNextIntermediateEquivalent;
        } else {
            mNextIntermediateUUID = NodeUUID::empty();
            mNextIntermediateEquivalent = 0;
        }
    } else {
        mPaymentExchangeHandler = nullptr;
        mIntermediateUUID = NodeUUID::empty();
        mIntermediateEquivalent = 0;
        mNextIntermediateUUID = NodeUUID::empty();
        mNextIntermediateEquivalent = 0;
    }
...
TransactionResult::SharedConst ReceiverPaymentTransaction::approve()
{
    ...
    if (mNextIntermediateEquivalent != 0) {
        stringstream s;
        s << mMessage->amount();
        auto amountInt = atoi(s.str().c_str());
        auto nextAmountInt = (int)(amountInt * mPaymentExchangeHandler->getRate(mEquivalent, mMessage->intermediateEquivalent()));
        auto nextCrossChainAmount = TrustLineAmount(nextAmountInt);
        info() << "mCross2ChainSignal: " << mIntermediateUUID << " " << mIntermediateEquivalent
               << " " << mNextIntermediateUUID << " " << mNextIntermediateEquivalent;
        mCross2ChainSignal(
                mIntermediateUUID,
                mIntermediateEquivalent,
                mNextIntermediateUUID,
                mNextIntermediateEquivalent,
                nextCrossChainAmount);
    } else if (mIntermediateEquivalent != 0) {
        stringstream s;
        s << mMessage->amount();
        auto amountInt = atoi(s.str().c_str());
        auto nextAmountInt = (int)(amountInt * mPaymentExchangeHandler->getRate(mEquivalent, mMessage->intermediateEquivalent()));
        auto nextCrossChainAmount = TrustLineAmount(nextAmountInt);
        info() << "mCrossChainSignal: " << mIntermediateUUID << " " << mIntermediateEquivalent;
        mCrossChainSignal(
                mIntermediateUUID,
                mIntermediateEquivalent,
                nextCrossChainAmount);
