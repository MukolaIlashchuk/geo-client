...
void TrustLinesManager::loadTrustLinesFromStorage()
{
    ...
    for (auto const &kTrustLine : kTrustLines) {
        ...
        info() << "Contractor " << kTrustLine->contractorNodeUUID();
        info() << "Contractor eth address " << kTrustLine->contractorEthereumAddress();
        info() << "eth channel id " << kTrustLine->ethereumChannelId();
        info() << "is allice " << kTrustLine->isAlice();
    }
}
    ...
    const string ethereumChannelId)
{
    ...
    trustLine->setEthereumChannelId(
        ethereumChannelId);
    updateEthereumTrustLine(
        ioTransaction,
        trustLine);
}

void TrustLinesManager::setContractorBitcoinPublicKey(
        IOTransaction::Shared ioTransaction,
        const NodeUUID &contractorUUID,
        const string contractorBitcoinPublicKey)
{
    if (not trustLineIsPresent(contractorUUID)) {
        throw NotFoundError(
                logHeader() + "::setContractorBitcoinPublicKey: "
                        "There is no trust line to contractor " + contractorUUID.stringUUID());
    }

    auto trustLine = mTrustLines[contractorUUID];
    trustLine->setContractorBtcoinPublicKey(
            contractorBitcoinPublicKey);
    updateBitcoinTrustLine(
            ioTransaction,
            trustLine);
}
...
const string TrustLinesManager::contractorBitcoinPublicKey(
        const NodeUUID &contractorUUID) const
{
    if (not trustLineIsPresent(contractorUUID)) {
        throw NotFoundError(
                logHeader() + "::contractorBitcoinPublicKey: "
                        "There is no trust line to contractor " + contractorUUID.stringUUID());
    }

    return mTrustLines.at(contractorUUID)->contractorBitcoinPubKey();
}
...
void TrustLinesManager::updateEthereumTrustLine(
    IOTransaction::Shared ioTransaction,
    TrustLine::Shared trustLine)
{
    if (not trustLineIsPresent(trustLine->contractorNodeUUID())) {
        throw NotFoundError(
            logHeader() + "::updateEthereumTrustLine: "
                    "There is no trust line to the contractor.");
    }
    ioTransaction->trustLinesHandler()->updateEthereumTrustLine(
        trustLine,
        mEquivalent);
    try {
        mTrustLines[trustLine->contractorNodeUUID()] = trustLine;
...
void TrustLinesManager::updateBitcoinTrustLine(
    IOTransaction::Shared ioTransaction,
    TrustLine::Shared trustLine)
{
    if (not trustLineIsPresent(trustLine->contractorNodeUUID())) {
        throw NotFoundError(
                logHeader() + "::updateBitcoinTrustLine: "
                        "There is no trust line to the contractor.");
    }
    ioTransaction->trustLinesHandler()->updateBitcoinTrustLine(
        trustLine,
        mEquivalent);
    try {
        mTrustLines[trustLine->contractorNodeUUID()] = trustLine;
