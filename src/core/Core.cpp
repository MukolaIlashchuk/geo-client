...
int Core::run()
{
    ...
    info() << "ERC20Address: " << mErc20Address;
    info() << "BtcPublicKey: " << mBitcoinPublicKey;
...
int Core::initSubsystems()
{
    ...
    try {
        mErc20Address = mSettings->erc20(&conf);
    ...
    try {
        mBitcoinPublicKey = mSettings->bitcoinPublicKey(&conf);
    ...
    try {
        rates = mSettings->rates(&conf);
    } catch (RuntimeError &) {
        cerr << utc_now() <<" : ERROR\tCORE\tCan't read rates from the settings" << endl;
    }
...
//    }
    ...
    initCode = initPaymentExchangeHandler();
    if (initCode != 0) {
        return initCode;
    }

    for (auto rate : rates) {
        auto from = (SerializedEquivalent)rate.at(0);
        auto to = (SerializedEquivalent)rate.at(1);
        double rt = rate.at(2);
        mPaymentExchangeHandler->setRate(from, to, rt);
    }
...
int Core::initPaymentExchangeHandler()
{
    try {
        mPaymentExchangeHandler = make_unique<PaymentExchangeHandler>(
            *mLog);
        info() << "Payments exchange handler is successfully initialized";
        return 0;
    } catch (const std::exception &e) {
        mLog->logException("Core", e);
        return -1;
